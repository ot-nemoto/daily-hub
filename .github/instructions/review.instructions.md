...
- `src/lib/` のカスタムエラー（`ForbiddenError` / `NotFoundError` / `ConflictError` 等）は catch して `{ error: message }` を返す。それ以外は再 throw する
...
- `VIEWER`: 閲覧のみ可能。日報作成・編集・コメント削除など書き込み操作は不可
- `MEMBER`: 自分の日報作成・編集、コメント追加・削除が可能
- `ADMIN`: すべてのデータ操作可能（ユーザー管理含む）

### 違反チェック

| 違反内容 | 重大度 |
|---|---|
| VIEWER ロールが日報作成・編集などの書き込み操作を実行できる実装になっている | **BLOCKER** |
| MEMBER が他ユーザーの日報を編集・削除できる実装になっている | **BLOCKER** |
...
| `ForbiddenError` | 権限はあるが操作が禁止されている場合（他ユーザーのデータ操作など） |
