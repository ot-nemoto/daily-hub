...
import { jsonError, serializeReport, statusForError, unauthorized } from "@/lib/api-response";
...
export async function PATCH(req: NextRequest, { params }: RouteContext) {
  ...
  try {
    const updated = await updateReport({ id, authorId: user.id, ...parsed.data });
    return NextResponse.json(serializeReport(updated));
  } catch (error) {
    if (error instanceof NotFoundError)
      return jsonError("日報が見つかりません", statusForError(error));
    if (error instanceof ForbiddenError) {
      return jsonError("この日報を編集する権限がありません", statusForError(error));
    }
    throw error;
  }
}
...
export async function DELETE(req: NextRequest, { params }: RouteContext) {
  ...
  if (user.role === "VIEWER") {
    return jsonError("日報を削除する権限がありません", 403);
  }
  ...
  try {
    ...
    if (error instanceof NotFoundError)
      return jsonError("日報が見つかりません", statusForError(error));
    if (error instanceof ForbiddenError) {
      return jsonError("この日報を削除する権限がありません", statusForError(error));
    }
    throw error;
  }
