...
| RPT-9 | 提出率は平日（月〜金）のみを分母とし、個人の休日登録がある日および祝日（全ユーザー共通）は分母から除外する |
...
有効ユーザー × 日付のマトリクスで日報の提出状況を一覧表示する。全ロールが閲覧できる。行ごとに提出率を表示し、休日・祝日は分母から除外する（「機能要件」RPT-8 / RPT-9）。

**URL 契約**: `?base=YYYY-MM-DD&period=<幅>`。`base` は表示範囲の右端（デフォルトは今日・UTC）、`period` は表示幅（`1w` / `2w` / `1m` / `1.5m` / `2m` / `3m`、デフォルトは `2w`）。不正値はデフォルトにフォールバックする。表示幅ごとの日数はコードを正とする。

期間指定は「表示幅を選び、窓を前後にスライドさせる」モデルで見せる（基準日から過去にさかのぼる、という説明を UI 上で不要にする）。

| 機能 | 説明 |
|------|------|
| 表示幅ボタン | 日数表記（`7日` / `14日` / …）のトグル。選択中を強調表示。幅を変えても右端（`base`）は維持する |
| 表示範囲 | `M/D(曜) 〜 M/D(曜)` で現在の範囲を表示。クリックすると終了日の日付入力（`<input type="date">`・上限は今日）に切り替わり、範囲内の完全な日付が入力された時点（または Enter）で右端を移動して閉じる。年の打ちかけなど範囲外の中間値では確定しない。Escape / フォーカス外で閉じる |
| ◀ / ▶ | 表示幅ぶん過去 / 未来へスライドする。未来方向は今日を超えず（超える場合は今日に丸める）、右端が今日のとき ▶ は無効 |
| 今日 | 右端を今日に戻す（幅は維持）。右端がすでに今日なら無効 |
| マトリクス | 列見出しは `M/D(曜)`（土=青・日=赤・祝日=赤＋祝日名）。セルは個人の休日「休」→ 提出済み「✓」→ 未提出「—」の順で表示。週末はグレー・祝日は赤系の列背景 |
| 提出率 | ユーザー名の横に `NN%`（切り捨て）。平日のみを分母とし、個人の休日・祝日を除外する |
| スクロール | 見出し行と左端のユーザー列を固定し、初期表示で右端（最新日）まで自動スクロールする |

祝日の表示仕様の詳細は「祝日管理」の「提出状況ビューへの反映」を参照。
...
ADMIN のみアクセス可能（他ロールは `/` へリダイレクト）。全ユーザーに無条件で適用されるグローバルな祝日を、カレンダー UI で登録・解除する。個人の休日（`/day-off`）とは別物で、`User` に紐づかない。

| 機能 | 説明 |
|------|------|
| 月ナビゲーション | 「◀ / ▶」ボタンで前月・翌月へ移動。初期表示は今月 |
| 日付セル | クリックで名称入力ポップオーバーを開く。登録済みは赤系（`bg-red-100 text-red-700`）で強調し名称を併記、当日は枠線で示す |
| 名称入力ポップオーバー | 選択日の「M/D の祝日」見出し・名称入力（任意・最大50文字）・「登録／更新」ボタンを表示。登録済みなら「解除」ボタンも表示。× で閉じる |
| 登録／更新 | 名称込みで登録（同日は名称を上書き更新）。名称は空欄でも登録可 |

- 祝日は全ユーザー共通で、`Holiday` の `date` ユニーク制約により1日1件
- 設定した祝日は提出状況ビューに反映される（下記）

**提出状況ビュー（`/reports/status`）への反映**

- 祝日は全ユーザー共通（列の属性）なので、週末と同じく列単位で示し、セルにバッジは出さない
  - 列見出しは日曜と同じ赤色で、日付ラベルの下に祝日名（未設定なら「祝日」）を小さく表示する。列幅に収まらない名称は省略し、`title`（ツールチップ）で全文を確認できる
  - 各ユーザーのセルは赤系背景（週末はグレー背景）とし、表示は週末と同じルール（個人の休日「休」→ 提出済み「✓」→ 未提出「—」）に従う。祝日と個人の休日が重なった場合は「休」を表示する
- 提出率の分母から祝日を除外する（RPT-9）
- VIEWER を含む全ロールが提出状況ビューで祝日を確認できる（祝日管理画面自体は ADMIN のみ）
...
| 状態 | 表示内容 |
|------|---------|
| Normal | 期間指定 UI とマトリクスを表示 |
| Loading | 期間指定の操作中は「読み込み中...」を UI 右側に表示（遷移は `useTransition`） |
| Empty | 有効ユーザーがいない場合は「有効なユーザーがいません。」を表示（マトリクス非表示） |
...
| 状態 | 表示内容 |
|------|---------|
| Normal | 今月のカレンダーを表示。登録済み祝日は赤系（`bg-red-100 text-red-700`）で名称込み強調 |
| Empty | 祝日未登録でもカレンダーは常に表示（専用の空状態なし） |
| Submitting | ポップオーバーの登録／解除ボタンを処理中は無効化 |
| Error | Server Action 失敗時はカレンダー上部にエラーメッセージを表示 |
