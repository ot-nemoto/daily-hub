...
カレンダー UI で休日を登録・解除する。登録した休日は提出状況ビューの提出率算出から除外される（[`requirements.md` RPT-8/RPT-9](requirements.md)）。ADMIN は他ユーザーの休日も代理設定可能。
...
| コンポーネント | 種別 | 用途 |
|--------------|------|------|
| `DisplayFieldTabs` | Client Component | 表示フィールドの複数選択トグル。`DisplayFieldContext`（`Set<DisplayField>`）を更新。未選択可 | 日次ビュー, 月次ビュー |
...
| `ReportNewForm` | Client Component | 日報作成フォーム。送信後、登録日の月次ビュー（`/reports/monthly?from=...&to=...`）にリダイレクト |
| `ReportEditForm` | Client Component | 日報編集フォーム。`onSuccess`/`onCancel` 必須（モーダル専用） |
