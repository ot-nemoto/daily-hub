...
| ユーザー追加 | `/admin/users/new` | ヘッダーあり | ADMIN のみ |
| 招待リンク発行 | `/admin/invitations` | ヘッダーあり | ADMIN のみ |
...
flowchart TD
    ...
    ADMIN_USERS_NEW[ユーザー追加]:::screen
    ADMIN_INV[招待リンク発行]:::screen
    ...
    ADMIN_USERS -->|ユーザーを追加| ADMIN_USERS_NEW
    ADMIN_USERS_NEW -->|作成成功| ADMIN_USERS
    ADMIN_USERS_NEW -->|キャンセル| ADMIN_USERS
...
### ユーザー追加（`/admin/users/new`）

ADMIN が直接ユーザーを作成する。名前・メールアドレス・ロールを指定する。対象者が Clerk でログインすると DB ユーザーと自動紐付けされる。

### 招待リンク発行（`/admin/invitations`）

ADMIN が期限付き招待リンクを発行・管理する。

| 機能 | 説明 |
|------|------|
| 招待リンク発行 | メールアドレス（任意）を指定して発行。有効期限72時間 |
| 招待一覧 | 有効・使用済み・期限切れのステータスを色分けで表示 |
| URLコピー | 有効な招待リンクのみコピーボタンを表示。コピー後2秒間「コピー済み」表示 |
...
|------|---------|
| Normal | 空フォームを表示 |
| Submitting | 送信ボタンを「作成中...」に変更・無効化 |
| ValidationError | 各フィールド下にエラーメッセージを表示 |
| Error | フォーム上部に `<ErrorMessage>` を表示（409 重複メールなど） |

### 招待リンク発行（`/admin/invitations`）

| 状態 | 表示内容 |
|------|---------|
| Normal | 招待フォーム＋招待一覧を表示 |
| Empty（招待なし） | 「招待リンクはありません」を表示 |
| Submitting | 発行ボタンを「発行中...」に変更・無効化 |
| Error | フォーム上部に `<ErrorMessage>` を表示 |
