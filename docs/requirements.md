...
| # | 要件 |
|---|------|
| AUTH-1 | Clerk を使用したメールアドレス + パスワードでログインができる（サインアップは管理者による招待またはユーザー直接作成） |
...
| # | 要件 |
|---|------|
| ADM-6 | 管理者はユーザーを直接作成できる（名前・メールを指定。対象者が Clerk でログインすると自動紐付け） |
...
| 画面 | パス | 説明 |
|------|------|------|
| ログイン | `/login` | Clerk SignIn UI（メール・パスワード） |
| 設定 | `/settings` | 名前変更（全ユーザー） |
| 認証エラー | `/auth-error` | アカウント無効化などの認証エラー表示 |
