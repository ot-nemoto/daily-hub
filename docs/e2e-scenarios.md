...
| 11 | `tsukune@example.com` | 初期表示の表示フィールド切替を確認する | デフォルト選択 | 「感想/課題/問題点」のみ選択状態で、カードに感想/課題/問題点が表示される |
| 12 | `tsukune@example.com` | 「本日の作業」「明日の予定」をクリックして3つ選択する | 複数選択・固定順表示 | カードに本日の作業 → 明日の予定 → 感想/課題/問題点の固定順で3フィールドが表示される |
| 13 | `tsukune@example.com` | 選択中のフィールドをすべて解除する | 未選択（0個）可 | すべて非選択にでき、カードにフィールドが1つも表示されない |
...
| 7 | `tsukune@example.com` | 表示フィールド切替で複数選択・全解除を行う | 複数選択・固定順・未選択 | 日次ビューと同様に、選択フィールドが固定順で表示され、全解除で未選択にできる |
