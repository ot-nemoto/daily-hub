...
| # | ユーザー | 手順 | 確認観点 | 期待値 |
|---|---------|------|---------|-------|
| 1 | `tsukune@example.com` | `/settings` でAPIキー欄を確認する（シード後） | 既存キーのマスク表示 | APIキーがマスク表示される（シードで生成済みのため） |
| 2 | `tsukune@example.com` | 「表示」をクリックする | プレーンテキスト表示 | APIキーがプレーンテキストで表示される |
| 3 | `tsukune@example.com` | 「隠す」をクリックする | マスク表示への切り替え | APIキーがマスク表示に戻る |
| 4 | `tsukune@example.com` | ページをリロードする | リロード後のマスク表示 | リロード後もマスク表示で保持されている |
| 5 | `tsukune@example.com` | 「再生成」をクリックする | キーの更新 | 新しいキーが生成され、プレーンテキストで表示される |
