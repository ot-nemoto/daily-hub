...
クライアント → リクエスト
  → getSession() を呼び出し
    ...
    → DB 未存在の新規ユーザーは role="MEMBER" で自動作成
...
| `session.user.isActive` | DB の `users.is_active`（無効ユーザーは API で 403） |
...
3. メールで DB 検索
   a. 未存在 → role="MEMBER" で新規作成
   b. clerkId あり（他の Clerk ID に紐付き済み） → null（認証失敗）
