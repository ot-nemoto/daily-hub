...
セッションは JWT 戦略で管理し、`middleware.ts` で未認証ユーザーを `/login` にリダイレクトする。
...
| ファイル | 役割 |
|----------|------|
| `src/lib/auth.ts` | NextAuth の設定（providers, session, callbacks, pages） |
...
| `src/middleware.ts` | 未認証リダイレクト。`auth` を使ってセッション確認 |
...
`middleware.ts` は以下のパスを**除いた全ルート**を認証必須とする。
